...
def predict_from_folder(folder, model, input_size, class_names):
    ...
    for img in walkdir(folder):
        img_load = keras.utils.load_img(os.path.join(*img), target_size=input_size)
        img_ar = keras.utils.img_to_array(img_load)
        img_resize = tf.expand_dims(img_ar, 0)
        y_pred = model.predict(img_resize)
        prediction = class_names[np.argmax(y_pred)]
        predictions.append(prediction)
        pre_label = os.path.dirname(os.path.join(*img))
