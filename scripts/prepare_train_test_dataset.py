...
def main(data_folder, labels, output_data_folder):
    ...
    # Step 01
    with open(labels, 'r') as fl:
        csv_file = csv.reader(fl, delimiter=',')

        # Step 02
        count_avoid_header = 0
        
        for row in csv_file:
            if count_avoid_header != 0:
                
                path_subset = os.path.join(output_data_folder, row[2])
                if not os.path.isdir(path_subset):
                    os.mkdir(path_subset)
                
                path_class = os.path.join(path_subset, row[1])
                if not os.path.isdir(path_class):
                    os.mkdir(path_class)
                
                # Step 03
                path_img_origin = os.path.join(data_folder, row[0])
                path_img_destiny = os.path.join(path_class, row[0])
                if not os.path.exists(path_img_destiny):
                    os.link(path_img_origin, path_img_destiny)
                
            count_avoid_header = 1
